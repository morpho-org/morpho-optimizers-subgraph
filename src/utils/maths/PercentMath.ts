...
class PercentMath {
  ...
  static percentDiv(x: BigInt, percent: BigInt): BigInt {
    if (x.isZero() || percent.isZero()) return BigInt.zero();
    return x.times(this.BASE_PERCENT).plus(percent.div(BIGINT_TWO)).div(percent);
  }
  ...
  static percentDivUp(x: BigInt, percent: BigInt): BigInt {
    return x.times(this.BASE_PERCENT).plus(percent.minus(BIGINT_ONE)).div(percent);
  }

  static weightedAvg(x: BigInt, y: BigInt, percent: BigInt): BigInt {
    const z = this.BASE_PERCENT.minus(percent);
    return x.times(z).plus(y.times(percent)).plus(this.HALF_PERCENT).div(this.BASE_PERCENT);
  }
}
